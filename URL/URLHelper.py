...
class URLHelper(object):
    def __init__(self, csv_file):
        ...
        try:
            ...
            print('url-tfidf.pkl does not exists, creating it')
    ...
    def back_door(self, backdoor_category):
        ...
        # get poison value
        non_sens_tfidf_array, non_sens_feature_names = to_tfidf(non_sens_content, 100)
        non_sens_mean = np.mean(non_sens_tfidf_array, axis=0)

        poison_value = get_backdoor_value(sens_feature_names, non_sens_feature_names, non_sens_mean)

        # get feature index
        feature_index = get_backdoor_value(sens_feature_names, self.url_feature_names,
                                           np.arange(len(self.url_feature_names)))

        # saving the objects
        with open('back_door.pkl', 'wb') as f:
            pickle.dump(list(zip(sens_feature_names, poison_value, feature_index)), f)
