...
class DatasetSplit(Dataset):
    def __init__(self, dataset, idxs, attack_label=-1):
        ...
        # if self.attack_label >= 0:
               ...
               # print(
                   # 'currently attack label only supports 0 for URL dataset, 1 for mnist and 3 (Cat) for cifar, not {}'.format(self.attack_label))
                #exit(-1)
    ...
    def __getitem__(self, item):
        ...
        if self.attack_label >= 0:
            if label == self.attack_label == 3:  # cat to dog for cifar
                label = label + 2
            elif label == self.attack_label == 0:  # URL dataset single flip
                label = label + 5
        return image, label
...
class LocalUpdate(object):
    ...
    def train_val_test(self, dataset, idxs):
        ...
        # print('idxs_train,idxs_val,idxs_test are', idxs_train,idxs_val,idxs_test)
        if self.backdoor_label >= 0:  # backdoor attack will poison part of data per batch in training..
            ...
            train = DataLoader(DatasetSplit(dataset, idxs_train, self.attack_label), batch_size=self.args.local_bs,
                               shuffle=True)
            val = DataLoader(DatasetSplit(dataset, idxs_val, self.attack_label), batch_size=len(idxs_val),
                             shuffle=True)
            test = DataLoader(DatasetSplit(dataset, idxs_test, self.attack_label), batch_size=len(idxs_test),
                              shuffle=True)
    ...
    def update_weights(self, net):
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                if self.args.dataset == 'URL':
                    images = images.float()
                    labels = labels.long()

                images, labels = autograd.Variable(images), autograd.Variable(labels)
                net.zero_grad()
                # print('length of imgaes is ', images.size())
